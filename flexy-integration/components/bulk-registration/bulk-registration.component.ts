...
export class BulkRegistrationComponent implements OnInit {
  ...
  private async fetchCredentials(): Promise<T2MAccount> {
    ...
    try {
      ...
      this.alert.warning('Could not connect to T2M', error as string);
      this.isLoading = false;
  ...
  private async fetchDevices(): Promise<EwonFlexyStructure[]> {
    ...
    try {
      ...
      this.alert.danger('Platform is currently unavailable.', error as string); // TODO check feedback plausibility
      return null;
    }
  }
